...
export const StepResults = memo(function StepResults({
  results,
  onReset,
}: StepResultsProps) {
  const stats = useMemo(() => {
        ...
        <table className="table-full">
          <thead>
            <tr style={{ background: "var(--c-gray-50)" }}>
              {["Paciente", "Canal", "Resultado", "Detalle"].map((h) => (
                <th key={h} className="th" style={{ fontSize: 11 }}>
                  {h}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {results.map((r) => (
              <tr
                key={r.patientId}
                style={{ borderBottom: "1px solid var(--c-gray-100)" }}
              >
                <td className="td">
                  <span
                    style={{
                ...
                <td className="td">
                  {(() => {
                    ...
                    return CI ? <CI size={16} /> : CHANNEL_CFG[r.channel].label;
                  })()}
                </td>
                <td className="td">
                  <span
