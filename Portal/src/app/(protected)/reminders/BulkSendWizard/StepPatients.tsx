...
export const StepPatients = memo(function StepPatients({
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <div className="wizard-section-title" style={{ marginBottom: 2 }}>
            Seleccionar pacientes
          </div>
          <div className="patient-preview__detail">
            {selected.size} de {eligible.length} seleccionados
          </div>
        </div>
        <button
          onClick={toggleAll}
      ...
      >
        ...
        {eligible.map((p) => (
          ...
          >
            <div
              className={`checkbox-box${selected.has(p.id) ? " checkbox-box--checked" : ""}`}
            >
              {selected.has(p.id) && <ACTION_ICONS.confirm size={12} />}
            </div>
            <div
