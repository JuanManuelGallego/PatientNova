...
function SendModeAndPatientStep({
  sendMode,
  setMode,
  form,
  setForm,
...
function ChannelAndMessageStep({
  form,
  setForm,
  selectedPatient,
  set,
        ...
        <label className="form-label">
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {[
              ...
              TWILIO_CONFIG.PATIENT_APPOINTMENT_REMINDER_CONFIRMATION.template
                .replace("{{1}}", ` ${selectedPatient?.name ?? ""}`)
                .replace("{{2}}", getUserName(user)),
            ].map((tmpl) => (
              <button
                key={tmpl}
                onClick={() => setForm((f) => ({ ...f, message: tmpl }))}
        ...
        <>
          <label className="form-label">
            ...
            <textarea
              ...
              className="form-input form-input--textarea"
              style={{ minHeight: 100 }}
              value={TWILIO_CONFIG.PATIENT_APPOINTMENT_REMINDER_CONFIRMATION.template
                .replace("{{1}}", ` ${selectedPatient?.name ?? ""}`)
                .replace("{{2}}", getUserName(user))
                .replace("{{3}}", form.fecha)
                .replace("{{4}}", form.hora)}
              onChange={set("message")}
              placeholder={`Hola ${selectedPatient?.name ?? "{nombre}"}, le recordamos su cita próximamente. Por favor confirme su asistencia.`}
            />
